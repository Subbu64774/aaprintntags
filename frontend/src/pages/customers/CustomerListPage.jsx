...
export default function CustomerListPage() {
  ...
  const handleDelete = (id) => {
    api.delete(`/customers/${id}`)
      .then(() => { message.success('Customer deleted'); fetchData(pagination.current - 1, pagination.pageSize, search); })
  ...
  const columns = [
    ...
    { title: 'GST', dataIndex: 'gstNumber', key: 'gstNumber' },
    {
      ...
      render: (_, record) => (
        <Space>
          <Button icon={<EyeOutlined />} size="small" onClick={() => navigate(`/customers/${record.customerId}`)} />
          {!isViewer && (
            <Button icon={<EditOutlined />} size="small" onClick={() => navigate(`/customers/${record.customerId}/edit`)} />
          )}
          {!isViewer && (
            <Popconfirm title="Delete this customer?" onConfirm={() => handleDelete(record.customerId)}>
