...
export default function generateCompletedPdf(data, action = 'download') {
  ...
  pdf.setFontSize(10);
  pdf.setTextColor(...black);
  pdf.text('Payment Completed Report', W / 2, y + 3, { align: 'center' });
  y += 6;
  ...
  const tableBody = invoices.map((row, idx) => [
    ...
    { content: fmt(row.paidAmount), styles: { halign: 'right', textColor: green, fontStyle: 'bold' } },
    { content: fmt(row.pendingAmount), styles: { halign: 'right' } }, // pendingAmount = remaining
  ]);

  const totalInvoiced = invoices.reduce((s, r) => s + (r.invoiceAmount || 0), 0);
  const totalPaid = invoices.reduce((s, r) => s + (r.paidAmount || 0), 0);
  const totalRemaining = invoices.reduce((s, r) => s + (r.pendingAmount || 0), 0);
  ...
  autoTable(pdf, {
    ...
    tableWidth: CW,
    head: [['#', 'Customer Name', 'PO Number', 'Invoice Date', 'Invoice Number', 'Invoice Amt (₹)', 'Paid Amt (₹)', 'Remaining (₹)']],
    body: tableBody,
    foot: [[
      ...
      { content: fmt(totalPaid), styles: { halign: 'right', fontStyle: 'bold', textColor: green } },
      { content: fmt(totalRemaining), styles: { halign: 'right', fontStyle: 'bold' } },
    ]],
    theme: 'grid',
  ...
  drawCard('Total Paid', data.totalPaid, [246, 255, 237], green, ML + cardW + 4);
  drawCard('Remaining Amount', data.totalPending, [245, 245, 245], [80, 80, 80], ML + (cardW + 4) * 2);
  y += cardH + 8;
  ...
  if (action === 'view') {
    ...
    pdf.save(`PaymentCompleted_${safeName}_${str(data.fromDate)}_to_${str(data.toDate)}.pdf`);
  }
}
